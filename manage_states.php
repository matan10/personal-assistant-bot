...
class ManageStates {
    ...
    private static $waiting_for_phone_number_file = "./state/waiting_for_phone_number.txt";
    ...
    // Waiting for phone number when converting phone number to person.
    public static function get_waiting_for_phone_number() {
        $state = file_get_contents(self::$waiting_for_phone_number_file);
        
        return $state == "1" ? true : false;
    }

    public static function set_waiting_for_phone_number($state) {
        $data = $state == true ? "1" : "0";
        file_put_contents(self::$waiting_for_phone_number_file, $data);
    }
    ...
    public static function reset_states() {
        self::set_waiting_for_phone_number(false);
